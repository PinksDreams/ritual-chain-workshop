...
contract AIJudge is PrecompileConsumer {
    ...
    function getBatchJudgingPrompt(
        uint256 bountyId
    ...
    function getSubmissionBySubmitter(
        uint256 bountyId,
        address submitter
    ) external view bountyExists(bountyId) returns (Submission memory) {
        return submissions[bountyId][submitter];
    }

    function getSubmitters(
        uint256 bountyId
    ) external view bountyExists(bountyId) returns (address[] memory) {
        return submittersByBounty[bountyId];
    }

    function getValidSubmitters(
        uint256 bountyId
    ) external view bountyExists(bountyId) returns (address[] memory) {
        return validSubmittersByBounty[bountyId];
    }

    function getValidSubmitter(
        uint256 bountyId,
        uint256 index
    ) external view bountyExists(bountyId) returns (address) {
        require(index < validSubmittersByBounty[bountyId].length, "bad index");
        return validSubmittersByBounty[bountyId][index];
    }

    function validSubmissionCount(
        uint256 bountyId
    ) external view bountyExists(bountyId) returns (uint256) {
        return validSubmittersByBounty[bountyId].length;
    }
